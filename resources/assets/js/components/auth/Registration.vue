        ...
        <div class="panel-body">
            <form method="post" role="form" @submit.prevent="submit()" @keydown="form.errors.clear($event.target.name)">
                ...
                <div :class="['form-group', {'has-error': form.errors.has('email')}]">
                    ...
                    <input type="email" class="form-control" name="email" id="email" placeholder="E-mail" v-model="form.email">
                ...
                <div :class="['form-group', {'has-error': form.errors.has('password_confirmation')}]">
                    ...
                    <input type="password" class="form-control" name="password_confirmation" id="password_confirmation" placeholder="Confirm Password" v-model="form.password_confirmation">
            ...
            submit() {
                ...
                this.$http.post('register', data)
                    .then(response => {
                        ...
                        auth.setToken(response.body.access_token);
                        auth.fetchUser(this);
                        this.$router.replace('/');
                    })
                    .catch(response => {
